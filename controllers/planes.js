...
const getAllServices = async (req, res) => {
    try {
        ...
        res.status(500).json({msg: 'Hubo un error al obtener los servicios', erro})
    }
}
const getOneService = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({msg: 'Hubo un error al obtener el servicios', error})
    }
}
...
const editService = async (req, res) => {
    ...
    try {
        const serviceEdit = await ModelService.findOneAndUpdate({_id: req.params.id}, req.body, {new:true})
        res.status(200).json({msg:'Servicio editado correctamente', serviceEdit })
    } catch (error) {
...
const deleteService = async (req, res) => {
    ...
    try {
        const serviceDelete = await ModelService.findByIdAndDelete({_id:req.params.id})
        res.status(200).json({msg:'Servicio eliminado correctamente'})
    } catch (error) {
        res.status(500).json({msg: 'Hubo un error al eliminar el servicio', error})
    }
}
module.exports={
    getAllServices,
